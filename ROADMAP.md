...
- 🟢 **Done** — `createCustomToken` — mint a Firebase custom token for a uid, signed locally with a service account key
- 🟢 **Done** — `verifyIdToken` — verify a Firebase ID token and return its decoded claims
- 🟢 **Done** — `getUser` — fetch a user by uid
- 🟢 **Done** — `getUserByEmail` — fetch a user by email address
...
- 🟢 **Done** — `createUser` — create a new user
- 🟢 **Done** — `updateUser` — update an existing user's properties
- 🟢 **Done** — `setCustomUserClaims` — set or clear developer claims on a user
- 🟢 **Done** — `deleteUser` — delete a user by uid
- 🟡 **Planned** — `deleteUsers` — bulk-delete up to 1000 users, returns per-user success/failure — **Medium**
- 🟢 **Done** — `listUsers` — paginated listing of all users
...
- 🟢 **Done** — `createSessionCookie` — exchange a verified ID token for a long-lived session cookie
- 🟢 **Done** — `verifySessionCookie` — verify a session cookie and return its decoded claims
